...
export class GalleryScreen {
  ...
  private renderItem(item: MediaFile, index: number): string {
    ...
    return `
      <div class="gallery-item" data-index="${index}" data-path="${item.path}">
        ${item.isVideo
          ...
          : `<img src="capacitor://localhost/_capacitor_file_${item.path}" loading="lazy" alt="${item.name}" onerror="this.style.display='none'" />`
        }
    ...
    container.querySelectorAll('.gallery-item').forEach(el => {
      el.addEventListener('touchstart', () => {
        pressTimer = setTimeout(() => {
          ...
          this.confirmDelete(path, container);
        }, 600);
      });
  ...
  private confirmDelete(path: string, container: HTMLElement): void {
    const overlay = document.createElement('div');
    overlay.className = 'dialog-overlay';
    overlay.innerHTML = `
      <div class="dialog">
        <h2>Löschen</h2>
        <p>${path.split('/').pop()} löschen?</p>
